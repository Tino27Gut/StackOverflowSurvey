...
df["Dbs"] = df["DatabaseWantToWorkWith"].astype(str).apply(
    lambda a: a.split(";")
)
...
fields_to_keep = ["DatabaseWantToWorkWith_Id"] + list(df.iloc[:, -dbs_amount:])
df = df[fields_to_keep]

# Save the csv file again
name = "L6_dim_DbWantToWork"
